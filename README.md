...
或在 Finder 双击 `启动斗地主.command`。首次加载会预热模型，终端显示“已就绪”后浏览器自动打开游戏。默认优先使用 **http://127.0.0.1:8770**；若 8770 已被占用，会自动选择空闲端口，并打印和打开实际地址。
...
6. 直接出完是最高优先；其次是可证明无法被压的炸弹/王炸、且炸后剩一手合法牌的确定收尾。其他局面只允许距最佳综合分不超过 0.20、即时风险不高于最佳候选、模拟收益差不超过半个样本权重的候选。Laya 接收紧凑的局面与候选描述，执行允许候选中模型概率最高的一项。
...
王炸按一手组合估算，拆双王、拆炸弹和已有领出权时空炸会增加代价；对手只剩 1/2 张时减弱保留代价。地主第一手若有不拆控制牌的普通领出，会保留炸弹/王炸；确定收尾例外，满手炸弹也不会被强迫拆开。它是条件策略，不是“永远先出最小单张”。

发牌使用系统随机源均匀洗牌，再分为 17/17/17/3 张，没有按牌力筛选或暗中调牌。随机公平不保证每局牌力相近；地主拿底牌和强牌更容易叫地主也会影响观感。参见 [发牌审计](docs/DEAL_AUDIT.md)。

策略依据见 [实战原则调研](docs/STRATEGY_PRINCIPLES.md)，最新验证见 [首手王炸修复记录](docs/STRATEGY_FIX.md)，首版验证见 [验证说明](docs/VALIDATION.md)。
...
- **端口被占用**：默认启动会自动换到空闲端口，不需要结束占用端口的系统进程。明确指定 `--port` 时会保留该要求，冲突则提示退出；可改用 `--port 8771` 或省略该参数。服务先绑定端口再加载模型，使用终端“已就绪”显示的实际地址。
